...
4. If `.harness.json` has `notion_database_id` set, check Notion for new tasks: query the Notion MCP connector for
   pages in that database where `Project` matches `project_slug` and `Ready` is checked, pipe the mapped results
   through `scripts/harness.sh notion-diff` to drop anything already imported, and if any remain, ask the user which
   to add. Chosen ones go in via `scripts/harness.sh notion-import <file>` as `pending` — never claim/work them this
   turn. Best-effort: if Notion isn't configured or the query fails, skip silently and continue.
