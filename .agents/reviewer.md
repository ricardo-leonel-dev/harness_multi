...
9. Only after both `progress/review.md` exists on disk **and** `record-review` has run, reply with the
   one-line final response.
...
- ❌ Never send the one-line final response before `progress/review.md` has actually been written to
  ...
  creating the file.
- ❌ Never send the one-line final response before `record-review` has actually run — the same reasoning
  applies: describing the verdict is not the same as recording it, and `log-out` will refuse to close the
  session without it regardless of what `progress/review.md` says.
